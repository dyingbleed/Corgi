        ...
        override def withConnection(conn: Connection): Unit = {
          ...
          while (iterator.hasNext) { // 主循环，遍历分区
            ...
            JDBCUtil.withTransaction(conn, new WithConnection {
              ...
              override def withConnection(conn: Connection): Unit = {
                val keyBuf = new mutable.ListBuffer[(String, Int, java.lang.Object)] // 字段名、类型、值
                val insertBuf = new mutable.ListBuffer[(String, Int, java.lang.Object)] // 字段名、类型、值
                val updateBuf = new mutable.ListBuffer[(String, Int, java.lang.Object)] // 字段名、类型、值
                ...
                if (isPK) {
                  val sql =
                    s"""
                       ...
                       |ON DUPLICATE KEY
                       |UPDATE ${updateBuf.map(i => s"${i._1}=?").mkString(",")}
                    """.stripMargin

                    new QueryRunner().execute(conn, sql, (insertBuf ++: updateBuf).map(_._3):_*)
                } else {
                  val querySQL =
                    s"""
                      |SELECT
                  ...
                  if (new QueryRunner().query(conn, querySQL, new ScalarHandler[Long](1), keyBuf.map(_._3):_*) == 0) {
                    // 插入
                    val insertSQL =
                      s"""
                        |INSERT INTO $sinkDB.$sinkTable (${insertBuf.map(_._1).mkString(",")})
